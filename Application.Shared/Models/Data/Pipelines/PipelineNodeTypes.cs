...
public static class PipelineNodeTypes
{
    ...
    /// <summary>
    /// Reads named scalar values out of its input and publishes them as <c>{{ vars.* }}</c> for later steps.
    /// <para>
    /// The one step whose output is not a relation anybody reads — it passes its input through unchanged and
    /// exists for its side effect. That is a deliberate exception to "nodes produce relations", and it is
    /// confined to this one type so the rest of the engine keeps its shape.
    /// </para>
    /// </summary>
    public const string TransformCapture = "transform.capture";
...
/// <summary>
/// Where a field's value ends up, and therefore how a substituted <c>{{ }}</c> value must be escaped.
/// <para>
/// This exists because the values being substituted are not all ours. <c>run.date</c> is generated by this
/// app and safe to paste anywhere. <c>params.*</c> arrives in an API trigger's request body, and
/// <c>vars.*</c> is read out of whatever the source returned — both are <b>external data</b>. Pasting either
/// straight into a WHERE clause or a URL is an injection through the pipeline's own data path.
/// </para>
/// <para>
/// A field kind is not enough to decide this: <c>where</c>, <c>url</c> and <c>table</c> are all
/// <c>PipelineFieldKinds.Text</c> and need three different treatments. So the catalogue states it per field.
/// </para>
/// </summary>
public static class PipelineTokenContexts
{
    /// <summary>
    /// Substituted verbatim. The default, and correct for anything that is read by a person rather than
    /// parsed by a machine — an email subject, a label, a message body.
    /// </summary>
    public const string Plain = "plain";

    /// <summary>
    /// The value lands inside SQL. Escaped as a <b>literal</b>: quoted and quote-doubled unless it is a
    /// number. A variable can therefore be a value and never an identifier, a column name or a fragment of
    /// SQL — that restriction is the whole reason this is safe.
    /// </summary>
    public const string Sql = "sql";

    /// <summary>The value lands in a URL or a query string. Percent-encoded.</summary>
...
public static class PipelineCaptureEmptyBehaviour
{
    /// <summary>
    /// Fail the step. The default: a variable that silently becomes empty is a filter that silently matches
    /// nothing or a URL that silently points at the wrong thing, and neither announces itself.
    /// </summary>
    public const string Fail = "fail";

    /// <summary>Publish each variable as an empty value and carry on.</summary>
    public const string Empty = "empty";

    public static readonly string[] All = [Fail, Empty];
}
